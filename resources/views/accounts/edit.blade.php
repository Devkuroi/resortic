...
<div class="page-header">
    <h4><i class="bi bi-pencil-square me-2"></i>Editar cuenta</h4>
    <p>Modifica los datos de <strong>{{ $account->name }}</strong>.</p>
</div>
    ...
    <div class="col-12 col-lg-7">
        <div class="card border-0 shadow-sm">
            <div class="card-body p-4">
                ...
                <form action="{{ route('accounts.update', $account) }}" method="POST" novalidate>
                    @csrf
                    @method('PUT')
                    ...
                    <div class="row g-3 mb-4">
                        <div class="col-md-6">
                            ...
                            <select id="role" name="role"
                                ...
                                <option value="admin"  {{ old('role', $account->role) == 'admin'  ? 'selected' : '' }}>Administrador</option>
                                <option value="hotel"  {{ old('role', $account->role) == 'hotel'  ? 'selected' : '' }}>Hotel</option>
                                <option value="client" {{ old('role', $account->role) == 'client' ? 'selected' : '' }}>Cliente</option>
                            </select>
                            @error('role')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
